...
from ..quantize import optimize_model
from ..quantize import ONNXQuantizer,ONNXModel
...
def generate_qat_model(model_names):
    ...
    graph = helper.make_graph(
      [ 
        ...
        helper.make_node("QuantizeLinear",["add_out","quant0_scale_const","quant0_zp_const"],["quant0_out"],"qlinear0"),
        helper.make_node("DequantizeLinear",["quant0_out","dequant0_scale_const","dequant0_zp_const"],["dequant0_out"],"dqlinear0"),
    ...
    graph = helper.make_graph(
      [ 
        ...
        helper.make_node("QuantizeLinear",["maxpool_out","quant0_scale_const","quant0_zp_const"],["quant0_out"],"qlinear0"),
        helper.make_node("DequantizeLinear",["quant0_out","dequant0_scale_const","dequant0_zp_const"],["dequant0_out"],"dqlinear0"),
        helper.make_node("Conv",["dequant0_out"],["conv0_out"],"conv0"),
        helper.make_node("QuantizeLinear",["maxpool_out","quant1_scale_const","quant1_zp_const"],["quant1_out"],"qlinear1"),
        helper.make_node("DequantizeLinear",["quant1_out","dequant1_scale_const","dequant1_zp_const"],["dequant1_out"],"dqlinear1"),
      ...
      for init_2 in model_2.graph.initializer:
        if init_2.name == init_1.name:
          ...
          if not np.array_equal(init1_arr,init2_arr):
            check_2 = False
            print("Error:  Initializer {} in test model dismatches with the expected model.".format(init_2.name))
          break
