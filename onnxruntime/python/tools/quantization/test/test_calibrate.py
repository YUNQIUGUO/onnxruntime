...
class TestCalibrate(unittest.TestCase):
    ...
    def test_augment_graph(self):
        ...
        graph = helper.make_graph([conv_node, clip_node, matmul_node], 'test_graph', [A,B], [E])
        ...
        augmented_model_path = './augmented_test_model.onnx'
        ...
        graph = helper.make_graph([conv_node_1, conv_node_2], 'test_graph_twoConv', [F], [H])
        model = helper.make_model(graph)
        test_model_path = './test_model_twoConv.onnx'
        onnx.save(model, test_model_path)

        # Augmenting graph
        data_reader = TestDataReader('./test_image_folder')
        augmented_model_path = './augmented_test_model_twoConv.onnx'
        calibrater = ONNXCalibrater(test_model_path, data_reader,['Conv','MatMul'], [], [], augmented_model_path, {})
        augmented_model = calibrater.augment_graph()
        onnx.save(augmented_model, augmented_model_path)
    ...
    #def test_quantization_params_calculation(self):
